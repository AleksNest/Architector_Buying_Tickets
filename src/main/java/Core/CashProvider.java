...
public class CashProvider {
    ...
    public boolean buy(Ticket ticket, User user) throws RuntimeException {
        this.cardNumber = user.getCardNumber();
        System.out.println("Number of card of "+ user.getUserName() + " is " + cardNumber);
        System.out.println("---------------------------- ");
    ...
    public void authorization(User client) {
        ...
        isAuthorized = false;
        for (BankAccount clients : CashRepository.getCashRepository().getClients()) {
            if (client.getCardNumber() == clients.getCard()) {
                isAuthorized = true;
                break;
            }
        }
